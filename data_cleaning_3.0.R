...
#string columns merge with "_"
str_cols <- c("eDisposition_24","eResponse_09","eResponse_10","eResponse_12","ePatient_14")
...
time_cols <- c("eDisposition_25")

#eDisposition_24 and eDisposition_25
dbExecute(con, "CREATE OR REPLACE TABLE factpcrdestinationteam_clean AS
SELECT
    PcrKey,
    ...
    MAX(TRY_CAST(eDisposition_25 AS TIMESTAMP))
        AS latest_eDisposition_25,

    MIN(TRY_CAST(eDisposition_25 AS TIMESTAMP))
...
dbGetQuery(con, "SELECT eDisposition_24 FROM factpcrdestinationteam_clean
                  WHERE eDisposition_24
                  LIKE '%_%'")
