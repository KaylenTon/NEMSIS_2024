...
merge_duplicates <- function(see_plot_of_dupes = FALSE) {
    ...
    dupe_cols_mer <- dupe_cols_cl %>%
      ...
      summarise(
        ...
        #merge all other columns together
        across(-c(dt_of_dpaa_duration, datetime_of_destination_prearrival_alert_or_activation),
...
## GET COLUMNS CAUSING DUPLICATES ONLY - turn this into an optional section alter
dupe_cols <- c("PcrKey", frequency_tbl$diff_cols) #just get the names of the duplicate columns
dupe_cols_only <- duplicates[, (colnames(duplicates) %in% dupe_cols)] #subset the duplicates table with the names from "dupe_cols"
...
dupe_cols_mer <- dupe_cols_cl %>%
  ...
  summarise(
    ...
    #merge all other columns together
    across(-c(dt_of_dpaa_duration, datetime_of_destination_prearrival_alert_or_activation, EMSTransportTimeMin),
           ~paste(unique(na.omit(.)), collapse = "_")),
    .groups = "drop") %>%
  subset(select = -c(diff_cols))
